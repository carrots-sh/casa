...
casa manages secrets as age-encrypted files in your dotfiles repo. Encryption is
chezmoi's native age support; casa adds key management, per-file key choice, and
safe editing on top. The `age` CLI must be installed (`brew install age`).
...
casa hands the file to chezmoi's encrypted add, which seals it with the default
key. If you have more than one key, casa first asks which key should seal this
...
options on the configs side. See [configs](configs.md).
...
passphrase, not by any key. Alongside the first backup casa generates a
`run_once_before` restore script that runs on a fresh machine before anything
needs decrypting: it globs `.casa/keys/*.key.age`, prompts for each key's
passphrase, and restores the identities into `~/.config/casa/keys`. The script
is fully generic — no key names appear in it — and skips cleanly if `age` is
not installed yet or the key already exists.
...
plain chezmoi + age. casa does not attempt to protect keys from your own
